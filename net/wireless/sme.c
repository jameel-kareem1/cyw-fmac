...
/*
 * Software SME in cyw_cfg80211, using auth/assoc/deauth calls to the
 * driver. This is is for implementing nl80211's connect/disconnect
 * and wireless extensions (if configured.)
 */

struct cyw_cfg80211_conn {
	struct cyw_cfg80211_connect_params params;
...
static void cyw_cfg80211_sme_free(struct wireless_dev *wdev)
{
	if (!wdev->conn)
		return;
...
static int cyw_cfg80211_conn_scan(struct wireless_dev *wdev)
{
...
static int cyw_cfg80211_conn_do_work(struct wireless_dev *wdev,
				 enum nl80211_timeout_reason *treason)
{
	...
	case CFG80211_CONN_SCAN_AGAIN:
		return cyw_cfg80211_conn_scan(wdev);
	case CFG80211_CONN_AUTHENTICATE_NEXT:
		if (WARN_ON(!rdev->ops->auth))
			return -EOPNOTSUPP;
		wdev->conn->state = CFG80211_CONN_AUTHENTICATING;
		return cyw_cfg80211_mlme_auth(rdev, wdev->netdev,
	...
	case CFG80211_CONN_ASSOCIATE_NEXT:
		...
		err = cyw_cfg80211_mlme_assoc(rdev, wdev->netdev, params->channel,
					  params->bssid, params->ssid,
					  params->ssid_len, &req);
		if (err)
			cyw_cfg80211_mlme_deauth(rdev, wdev->netdev, params->bssid,
					     NULL, 0,
...
void cyw_cfg80211_conn_work(struct work_struct *work)
{
	struct cyw_cfg80211_registered_device *rdev =
		container_of(work, struct cyw_cfg80211_registered_device, conn_work);
	...
	list_for_each_entry(wdev, &rdev->wiphy.wdev_list, list) {
		...
		if (cyw_cfg80211_conn_do_work(wdev, &treason)) {
			...
			__cyw_cfg80211_connect_result(wdev->netdev, &cr, false);
		}
		wdev_unlock(wdev);
	}
...
static struct cyw_cfg80211_bss *cyw_cfg80211_get_conn_bss(struct wireless_dev *wdev)
{
	struct cyw_cfg80211_registered_device *rdev = wiphy_to_rdev(wdev->wiphy);
	...
	bss = cyw_cfg80211_get_bss(wdev->wiphy, wdev->conn->params.channel,
...
static void __cyw_cfg80211_sme_scan_done(struct net_device *dev)
{
	...
	bss = cyw_cfg80211_get_conn_bss(wdev);
	if (bss)
		cyw_cfg80211_put_bss(&rdev->wiphy, bss);
	else
		schedule_work(&rdev->conn_work);
}
...
void cyw_cfg80211_sme_scan_done(struct net_device *dev)
{
	...
	wdev_lock(wdev);
	__cyw_cfg80211_sme_scan_done(dev);
	wdev_unlock(wdev);
}
...
static int cyw_cfg80211_sme_get_conn_ies(struct wireless_dev *wdev,
				     ...
				     const u8 **out_ies, size_t *out_ies_len)
{
	...
	if (!rdev->wiphy.extended_capabilities_len ||
	    (ies && cyw_cfg80211_find_ie(WLAN_EID_EXT_CAPABILITY, ies, ies_len))) {
...
static int cyw_cfg80211_sme_connect(struct wireless_dev *wdev,
				...
				const u8 *prev_bssid)
{
	...
	if (cyw_cfg80211_sme_get_conn_ies(wdev, connect->ie, connect->ie_len,
				      &wdev->conn->ie,
	...
	if (bss) {
		...
		err = cyw_cfg80211_conn_do_work(wdev, &treason);
		cyw_cfg80211_put_bss(wdev->wiphy, bss);
	} else {
		/* otherwise we'll need to scan for the AP first */
		err = cyw_cfg80211_conn_scan(wdev);
...
static int cyw_cfg80211_sme_disconnect(struct wireless_dev *wdev, u16 reason)
{
	struct cyw_cfg80211_registered_device *rdev = wiphy_to_rdev(wdev->wiphy);
	int err;
	...
	err = cyw_cfg80211_mlme_deauth(rdev, wdev->netdev,
				   wdev->conn->params.bssid,
				   NULL, 0, reason, false);
 out:
	cyw_cfg80211_sme_free(wdev);
...
static bool cyw_cfg80211_is_all_idle(void)
{
	struct cyw_cfg80211_registered_device *rdev;
	struct wireless_dev *wdev;
	...
	list_for_each_entry(rdev, &cyw_cfg80211_rdev_list, list) {
		list_for_each_entry(wdev, &rdev->wiphy.wdev_list, list) {
			...
			if (wdev->conn || wdev->current_bss ||
			    cyw_cfg80211_beaconing_iface_active(wdev))
				is_all_idle = false;
			wdev_unlock(wdev);
		}
	}
...
{
	rtnl_lock();
	if (cyw_cfg80211_is_all_idle())
		regulatory_hint_disconnect();
	rtnl_unlock();
}

DECLARE_WORK(cyw_cfg80211_disconnect_work, disconnect_work);
			       ...
			       bool wextev)
{
	...
	if (WARN_ON(wdev->iftype != NL80211_IFTYPE_STATION &&
		...
		cyw_cfg80211_put_bss(wdev->wiphy, cr->bss);
		return;
	}
	...
	if (!cr->bss && (cr->status == WLAN_STATUS_SUCCESS)) {
		...
		cr->bss = cyw_cfg80211_get_bss(wdev->wiphy, NULL, cr->bssid,
					   wdev->ssid, wdev->ssid_len,
					   wdev->conn_bss_type,
					   IEEE80211_PRIVACY_ANY);
		if (cr->bss)
			cyw_cfg80211_hold_bss(bss_from_pub(cr->bss));
	}
	...
	if (cr->status != WLAN_STATUS_SUCCESS) {
		...
		if (cr->bss) {
			cyw_cfg80211_unhold_bss(bss_from_pub(cr->bss));
			cyw_cfg80211_put_bss(wdev->wiphy, cr->bss);
		}
		cyw_cfg80211_sme_free(wdev);
		return;
	...
	if (!(wdev->wiphy->flags & WIPHY_FLAG_HAS_STATIC_WEP))
		cyw_cfg80211_upload_connect_keys(wdev);
			   ...
			   gfp_t gfp)
{
	...
	if (params->bss) {
		struct cyw_cfg80211_internal_bss *ibss = bss_from_pub(params->bss);

		if (list_empty(&ibss->list)) {
			struct cyw_cfg80211_bss *found = NULL, *tmp = params->bss;

			found = cyw_cfg80211_get_bss(wdev->wiphy, NULL,
						 params->bss->bssid,
						 wdev->ssid, wdev->ssid_len,
						 wdev->conn_bss_type,
						 IEEE80211_PRIVACY_ANY);
			if (found) {
				 ...
				 */
				cyw_cfg80211_bss_update(rdev, ibss, false,
						    ibss->ts);
			}

			cyw_cfg80211_put_bss(wdev->wiphy, tmp);
		}
	}
	...
	if (!ev) {
		cyw_cfg80211_put_bss(wdev->wiphy, params->bss);
		return;
	}
	...
	if (params->bss)
		cyw_cfg80211_hold_bss(bss_from_pub(params->bss));
...
EXPORT_SYMBOL(cyw_cfg80211_connect_done);
		       ...
		       struct cyw_cfg80211_roam_info *info)
{
	...
	cyw_cfg80211_hold_bss(bss_from_pub(info->bss));
	...
	return;
out:
	cyw_cfg80211_put_bss(wdev->wiphy, info->bss);
}
		     ...
		     gfp_t gfp)
{
	...
	if (!info->bss) {
		info->bss = cyw_cfg80211_get_bss(wdev->wiphy, info->channel,
...
EXPORT_SYMBOL(cyw_cfg80211_roamed);
...
EXPORT_SYMBOL(cyw_cfg80211_port_authorized);
			     ...
			     size_t ie_len, u16 reason, bool from_ap)
{
	...
	schedule_work(&cyw_cfg80211_disconnect_work);
}
...
EXPORT_SYMBOL(cyw_cfg80211_disconnected);
		     ...
		     const u8 *prev_bssid)
{
	...
	cyw_cfg80211_oper_and_ht_capa(&connect->ht_capa_mask,
				  rdev->wiphy.ht_capa_mod_mask);
	cyw_cfg80211_oper_and_vht_capa(&connect->vht_capa_mask,
	...
	if (!rdev->ops->connect)
		err = cyw_cfg80211_sme_connect(wdev, connect, prev_bssid);
	else
			...
			struct net_device *dev, u16 reason, bool wextev)
{
	...
	if (wdev->conn)
		err = cyw_cfg80211_sme_disconnect(wdev, reason);
	else if (!rdev->ops->disconnect)
		cyw_cfg80211_mlme_down(rdev, dev);
	else if (wdev->ssid_len)
		err = rdev_disconnect(rdev, dev, reason);

	/*
	 * Clear ssid_len unless we actually were fully connected,
	 * in which case cyw_cfg80211_disconnected() will take care of
	 * this later.
	 */
	if (!wdev->current_bss)
		wdev->ssid_len = 0;
...
void cyw_cfg80211_autodisconnect_wk(struct work_struct *work)
{
	...
	if (wdev->conn_owner_nlportid) {
		...
		case NL80211_IFTYPE_ADHOC:
			__cyw_cfg80211_leave_ibss(rdev, wdev->netdev, false);
			break;
		case NL80211_IFTYPE_AP:
		case NL80211_IFTYPE_P2P_GO:
			__cyw_cfg80211_stop_ap(rdev, wdev->netdev, false);
			break;
		case NL80211_IFTYPE_MESH_POINT:
			__cyw_cfg80211_leave_mesh(rdev, wdev->netdev);
			break;
		...
		case NL80211_IFTYPE_P2P_CLIENT:
			 ...
			 * use cyw_cfg80211_disconnect.
			 */
			if (rdev->ops->disconnect || wdev->current_bss)
				cyw_cfg80211_disconnect(rdev, wdev->netdev,
						    WLAN_REASON_DEAUTH_LEAVING,
						    true);
			else
				cyw_cfg80211_mlme_deauth(rdev, wdev->netdev,
