				   ...
				   bool pairwise, const u8 *mac_addr)
{
	...
	if (!cyw_cfg80211_supported_cipher_suite(&rdev->wiphy, params->cipher))
		return -EINVAL;
...
EXPORT_SYMBOL(cyw_cfg80211_classify8021d);
...
const struct element *ieee80211_bss_get_elem(struct cyw_cfg80211_bss *bss, u8 id)
{
	...
	return cyw_cfg80211_find_elem(id, ies->data, ies->len);
}
...
void cyw_cfg80211_process_wdev_events(struct wireless_dev *wdev)
{
	...
	while (!list_empty(&wdev->event_list)) {
		ev = list_first_entry(&wdev->event_list,
				      struct cyw_cfg80211_event, list);
		list_del(&ev->list);
		...
		case EVENT_CONNECT_RESULT:
			__cyw_cfg80211_connect_result(
				wdev->netdev,
				&ev->cr,
				ev->cr.status == WLAN_STATUS_SUCCESS);
			break;
		case EVENT_ROAMED:
			__cyw_cfg80211_roamed(wdev, &ev->rm);
			break;
		case EVENT_DISCONNECTED:
			__cyw_cfg80211_disconnected(wdev->netdev,
						ev->dc.ie, ev->dc.ie_len,
						ev->dc.reason,
						!ev->dc.locally_generated);
			break;
		case EVENT_IBSS_JOINED:
			__cyw_cfg80211_ibss_joined(wdev->netdev, ev->ij.bssid,
					       ev->ij.channel);
			break;
		case EVENT_STOPPED:
			__cyw_cfg80211_leave(wiphy_to_rdev(wdev->wiphy), wdev);
			break;
		case EVENT_PORT_AUTHORIZED:
			__cyw_cfg80211_port_authorized(wdev, ev->pa.bssid);
			break;
		}
		wdev_unlock(wdev);
			  ...
			  struct vif_params *params)
{
	...
	if (ntype != otype) {
		...
		case NL80211_IFTYPE_AP:
			cyw_cfg80211_stop_ap(rdev, dev, true);
			break;
		case NL80211_IFTYPE_ADHOC:
			cyw_cfg80211_leave_ibss(rdev, dev, false);
			break;
		...
		cyw_cfg80211_mlme_purge_registrations(dev->ieee80211_ptr);
	}
	...
	if (!err && ntype != otype && netif_running(dev)) {
		cyw_cfg80211_update_iface_num(rdev, ntype, 1);
		cyw_cfg80211_update_iface_num(rdev, otype, -1);
	}

	return err;
}

static u32 cyw_cfg80211_calculate_bitrate_ht(struct rate_info *rate)
{
...
static u32 cyw_cfg80211_calculate_bitrate_dmg(struct rate_info *rate)
{
...
static u32 cyw_cfg80211_calculate_bitrate_edmg(struct rate_info *rate)
{
	static const u32 __mcs2bitrate[] = {
		/* control PHY */
		[0] =   275,
...
static u32 cyw_cfg80211_calculate_bitrate_vht(struct rate_info *rate)
{
...
static u32 cyw_cfg80211_calculate_bitrate_he(struct rate_info *rate)
{
#define SCALE 2048
	u16 mcs_divisors[12] = {
		34133, /* 16.666666... */
...
u32 cyw_cfg80211_calculate_bitrate(struct rate_info *rate)
{
	if (rate->flags & RATE_INFO_FLAGS_MCS)
		return cyw_cfg80211_calculate_bitrate_ht(rate);
	if (rate->flags & RATE_INFO_FLAGS_DMG)
		return cyw_cfg80211_calculate_bitrate_dmg(rate);
	if (rate->flags & RATE_INFO_FLAGS_EDMG)
		return cyw_cfg80211_calculate_bitrate_edmg(rate);
	if (rate->flags & RATE_INFO_FLAGS_VHT_MCS)
		return cyw_cfg80211_calculate_bitrate_vht(rate);
	if (rate->flags & RATE_INFO_FLAGS_HE_MCS)
		return cyw_cfg80211_calculate_bitrate_he(rate);

	return rate->legacy;
}
EXPORT_SYMBOL(cyw_cfg80211_calculate_bitrate);
...
EXPORT_SYMBOL(cyw_cfg80211_get_p2p_attr);
...
static void cyw_cfg80211_calculate_bi_data(struct wiphy *wiphy, u32 new_beacon_int,
				       u32 *beacon_int_gcd,
				 ...
				 enum nl80211_iftype iftype, u32 beacon_int)
{
	 ...
	 * to cyw_cfg80211_check_combinations(), in which case we'll validate more
	 * through the cyw_cfg80211_calculate_bi_data() call and code in
	 * cyw_cfg80211_iter_combinations().
	 */
			       ...
			       void *data)
{
	 ...
	 * cyw_cfg80211 already - the only thing not would appear to be any new
	 * interfaces (while being brought up) and channel/radar data.
	 */
	cyw_cfg80211_calculate_bi_data(wiphy, params->new_beacon_int,
	...
	for (iftype = 0; iftype < NUM_NL80211_IFTYPES; iftype++) {
		...
		if (params->iftype_num[iftype] > 0 &&
		    !cyw_cfg80211_iftype_allowed(wiphy, iftype, 0, 1))
	...
	for (i = 0; i < wiphy->n_iface_combinations; i++) {
		...
		for (iftype = 0; iftype < NUM_NL80211_IFTYPES; iftype++) {
			if (cyw_cfg80211_iftype_allowed(wiphy, iftype, 0, 1))
				continue;
...
EXPORT_SYMBOL(cyw_cfg80211_iter_combinations);

static void
cyw_cfg80211_iter_sum_ifcombs(const struct ieee80211_iface_combination *c,
			  void *data)
{
	int *num = data;
	(*num)++;
				...
				struct iface_combination_params *params)
{
	...
	err = cyw_cfg80211_iter_combinations(wiphy, params,
					 cyw_cfg80211_iter_sum_ifcombs, &num);
	if (err)
		return err;
	if (num == 0)
		return -EBUSY;
...
EXPORT_SYMBOL(cyw_cfg80211_check_combinations);
...
EXPORT_SYMBOL(cyw_cfg80211_get_station);
...
EXPORT_SYMBOL(cyw_cfg80211_free_nan_func);
...
EXPORT_SYMBOL(cyw_cfg80211_sinfo_alloc_tid_stats);
...
EXPORT_SYMBOL(cyw_cfg80211_send_layer2_update);
...
EXPORT_SYMBOL(cyw_cfg80211_iftype_allowed);
