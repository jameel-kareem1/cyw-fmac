...
const struct mesh_setup default_mesh_setup = {
	/* cyw_cfg80211_join_mesh() will pick a channel if needed */
			 ...
			 const struct mesh_config *conf)
{
	...
	if (!setup->basic_rates) {
		...
		if (setup->chandef.chan->band == NL80211_BAND_2GHZ) {
			...
			scan_width = cyw_cfg80211_chandef_to_scan_width(&setup->chandef);
			setup->basic_rates = ieee80211_mandatory_rates(sband,
								       scan_width);
		}
	}

	err = cyw_cfg80211_chandef_dfs_required(&rdev->wiphy,
					    &setup->chandef,
	...
	if (!cyw_cfg80211_reg_can_beacon(&rdev->wiphy, &setup->chandef,
				     NL80211_IFTYPE_MESH_POINT))
		return -EINVAL;
			...
			struct net_device *dev)
{
	...
	wdev_lock(wdev);
	err = __cyw_cfg80211_leave_mesh(rdev, dev);
	wdev_unlock(wdev);
