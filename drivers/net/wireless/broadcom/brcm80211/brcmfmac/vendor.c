...
static int brcmf_cyw_cfg80211_vndr_cmds_dcmd_handler(struct wiphy *wiphy,
						 ...
						 const void *data, int len)
{
	...
	while (ret_len > 0) {
		...
		reply = cyw_cfg80211_vendor_cmd_alloc_reply_skb(wiphy, payload);
		if (NULL == reply) {
			ret = -ENOMEM;
			break;
		}
		...
		ret = cyw_cfg80211_vendor_cmd_reply(reply);
		if (ret)
			break;
...

{
	...
	skb = cyw_cfg80211_vendor_event_alloc(wiphy, NULL,
					  sizeof(*phy_temp_evt),
	...
	cyw_cfg80211_vendor_event(skb, GFP_KERNEL);
	return 0;
}
...
const struct wiphy_vendor_command brcmf_vendor_cmds[] = {
	{
		...
		.doit = brcmf_cyw_cfg80211_vndr_cmds_dcmd_handler
	},
};
