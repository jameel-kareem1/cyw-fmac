...
struct brcmf_pno_info {
	int n_reqs;
	struct cyw_cfg80211_sched_scan_request *reqs[BRCMF_PNO_MAX_BUCKETS];
...
static int brcmf_pno_add_ssid(struct brcmf_if *ifp, struct cyw_cfg80211_ssid *ssid,
			      bool active)
{
	struct brcmf_pub *drvr = ifp->drvr;
...
static bool brcmf_is_ssid_active(struct cyw_cfg80211_ssid *ssid,
				 struct cyw_cfg80211_sched_scan_request *req)
{
	int i;
...
static int brcmf_pno_get_bucket_channels(struct cyw_cfg80211_sched_scan_request *r,
...
static int brcmf_pno_prep_fwconfig(struct brcmf_pno_info *pi,
				   ...
				   u32 *scan_freq)
{
	struct cyw_cfg80211_sched_scan_request *sr;
...
{
	struct cyw_cfg80211_sched_scan_request *r;
	struct cyw_cfg80211_match_set *ms;
	bool active;
	int i, j, err = 0;
...
int brcmf_pno_attach(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pno_info *pi;
...
void brcmf_pno_detach(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pno_info *pi;
