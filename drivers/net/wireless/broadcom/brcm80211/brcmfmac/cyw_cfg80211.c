...
static bool check_vif_up(struct brcmf_cyw_cfg80211_vif *vif)
{
...
/* This is to override regulatory domains defined in cyw_cfg80211 module (reg.c)
...
static u16 chandef_to_chanspec(struct brcmu_d11inf *d11inf,
			       struct cyw_cfg80211_chan_def *ch)
{
...
static int brcmf_vif_change_validate(struct brcmf_cyw_cfg80211_info *cfg,
				     struct brcmf_cyw_cfg80211_vif *vif,
				     enum nl80211_iftype new_type)
{
	struct brcmf_cyw_cfg80211_vif *pos;
	...
	if (check_combos)
		ret = cyw_cfg80211_check_combinations(cfg->wiphy, &params);

	return ret;
}

static int brcmf_vif_add_validate(struct brcmf_cyw_cfg80211_info *cfg,
				  enum nl80211_iftype new_type)
{
	struct brcmf_cyw_cfg80211_vif *pos;
	struct iface_combination_params params = {
	...
	params.iftype_num[new_type]++;
	return cyw_cfg80211_check_combinations(cfg->wiphy, &params);
}
...
static void
brcmf_cyw_cfg80211_update_proto_addr_mode(struct wireless_dev *wdev)
{
	struct brcmf_cyw_cfg80211_vif *vif;
	struct brcmf_if *ifp;

	vif = container_of(wdev, struct brcmf_cyw_cfg80211_vif, wdev);
	ifp = vif->ifp;
...
static int brcmf_cyw_cfg80211_request_ap_if(struct brcmf_if *ifp)
{
	struct brcmf_pub *drvr = ifp->drvr;
...
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(cfg_to_ndev(cfg));
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_cyw_cfg80211_vif *vif;
	int err;

	if (brcmf_cyw_cfg80211_vif_event_armed(cfg))
	...
	brcmf_cyw_cfg80211_arm_vif_event(cfg, vif);

	err = brcmf_cyw_cfg80211_request_ap_if(ifp);
	if (err) {
		brcmf_cyw_cfg80211_arm_vif_event(cfg, NULL);
		goto fail;
	}

	/* wait for firmware event */
	err = brcmf_cyw_cfg80211_wait_vif_event(cfg, BRCMF_E_IF_ADD,
...
static bool brcmf_is_apmode(struct brcmf_cyw_cfg80211_vif *vif)
{
...
static bool brcmf_is_ibssmode(struct brcmf_cyw_cfg80211_vif *vif)
{
	return vif->wdev.iftype == NL80211_IFTYPE_ADHOC;
}

static struct wireless_dev *brcmf_cyw_cfg80211_add_iface(struct wiphy *wiphy,
						     const char *name,
						     ...
						     struct vif_params *params)
{
	...
	if (IS_ERR(wdev))
		...
		brcmf_cyw_cfg80211_update_proto_addr_mode(wdev);
...
s32 brcmf_notify_escan_complete(struct brcmf_cyw_cfg80211_info *cfg,
				struct brcmf_if *ifp, bool aborted,
				bool fw_abort)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_scan_params_le params_le;
	struct cyw_cfg80211_scan_request *scan_request;
	u64 reqid;
	u32 bucket;
	s32 err = 0;
	...
	/* to this functon and might cause a double cyw_cfg80211_scan_done      */
	...
	if (cfg->int_escan_map) {
		...
		while (cfg->int_escan_map) {
			...
			if (!aborted) {
				brcmf_dbg(SCAN, "report results: reqid=%llu\n",
					  reqid);
				cyw_cfg80211_sched_scan_results(cfg_to_wiphy(cfg),
							    reqid);
			}
		}
		...
		cyw_cfg80211_scan_done(scan_request, &info);
	}
...
static int brcmf_cyw_cfg80211_del_ap_iface(struct wiphy *wiphy,
				       struct wireless_dev *wdev)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	...
	brcmf_cyw_cfg80211_arm_vif_event(cfg, ifp->vif);
	...
	/* wait for firmware event */
	ret = brcmf_cyw_cfg80211_wait_vif_event(cfg, BRCMF_E_IF_DEL,
					    BRCMF_VIF_EVENT_TIMEOUT);
	if (!ret) {
		bphy_err(drvr, "timeout occurred\n");
		err = -EIO;
...
static
int brcmf_cyw_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct net_device *ndev = wdev->netdev;
	...
	case NL80211_IFTYPE_MESH_POINT:
		return -EOPNOTSUPP;
	case NL80211_IFTYPE_AP:
		return brcmf_cyw_cfg80211_del_ap_iface(wiphy, wdev);
	case NL80211_IFTYPE_P2P_CLIENT:
	case NL80211_IFTYPE_P2P_GO:
	case NL80211_IFTYPE_P2P_DEVICE:
...
static s32
brcmf_cyw_cfg80211_change_iface(struct wiphy *wiphy, struct net_device *ndev,
			 enum nl80211_iftype type,
			 struct vif_params *params)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_vif *vif = ifp->vif;
	struct brcmf_pub *drvr = cfg->pub;
	s32 infra = 0;
	s32 ap = 0;
	s32 err = 0;
	...
	if ((type == NL80211_IFTYPE_STATION) &&
		 ...
		 * normally. It will give a trace in cyw_cfg80211, but it is the
	...
	brcmf_cyw_cfg80211_update_proto_addr_mode(&vif->wdev);
...
static void brcmf_escan_prep(struct brcmf_cyw_cfg80211_info *cfg,
			     struct brcmf_scan_params_le *params_le,
...
static s32
brcmf_run_escan(struct brcmf_cyw_cfg80211_info *cfg, struct brcmf_if *ifp,
		struct cyw_cfg80211_scan_request *request)
{
	struct brcmf_pub *drvr = cfg->pub;
...
static s32
brcmf_do_escan(struct brcmf_if *ifp, struct cyw_cfg80211_scan_request *request)
{
	struct brcmf_cyw_cfg80211_info *cfg = ifp->drvr->config;
	s32 err;
...
static s32
brcmf_cyw_cfg80211_scan(struct wiphy *wiphy, struct cyw_cfg80211_scan_request *request)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_cyw_cfg80211_vif *vif;
	...
	vif = container_of(request->wdev, struct brcmf_cyw_cfg80211_vif, wdev);
	if (!check_vif_up(vif))
		return -EIO;
...
static s32 brcmf_cyw_cfg80211_set_wiphy_params(struct wiphy *wiphy, u32 changed)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
...
static void brcmf_init_prof(struct brcmf_cyw_cfg80211_profile *prof)
{
...
static void brcmf_link_down(struct brcmf_cyw_cfg80211_vif *vif, u16 reason,
			    bool locally_generated)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(vif->wdev.wiphy);
	struct brcmf_pub *drvr = cfg->pub;
	...
	if (test_and_clear_bit(BRCMF_VIF_STATUS_CONNECTED, &vif->sme_state)) {
		...
		if ((vif->wdev.iftype == NL80211_IFTYPE_STATION) ||
		    (vif->wdev.iftype == NL80211_IFTYPE_P2P_CLIENT))
			cyw_cfg80211_disconnected(vif->wdev.netdev, reason, NULL, 0,
...
static s32
brcmf_cyw_cfg80211_join_ibss(struct wiphy *wiphy, struct net_device *ndev,
		      struct cyw_cfg80211_ibss_params *params)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_profile *profile = &ifp->vif->profile;
	struct brcmf_pub *drvr = cfg->pub;
...
static s32
brcmf_cyw_cfg80211_leave_ibss(struct wiphy *wiphy, struct net_device *ndev)
{
	struct brcmf_if *ifp = netdev_priv(ndev);
...
{
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_profile *profile = ndev_to_prof(ndev);
	struct brcmf_pub *drvr = ifp->drvr;
	struct brcmf_cyw_cfg80211_security *sec;
	s32 val = 0;
	s32 err = 0;
...
static s32
brcmf_set_key_mgmt(struct net_device *ndev, struct cyw_cfg80211_connect_params *sme)
{
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_profile *profile = &ifp->vif->profile;
	struct brcmf_pub *drvr = ifp->drvr;
...
static s32
brcmf_cyw_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev,
		       struct cyw_cfg80211_connect_params *sme)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
...
static s32
brcmf_cyw_cfg80211_disconnect(struct wiphy *wiphy, struct net_device *ndev,
		       u16 reason_code)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
	...
	clear_bit(BRCMF_VIF_STATUS_CONNECTING, &ifp->vif->sme_state);
	cyw_cfg80211_disconnected(ndev, reason_code, NULL, 0, true, GFP_KERNEL);
...
static s32
brcmf_cyw_cfg80211_set_tx_power(struct wiphy *wiphy, struct wireless_dev *wdev,
...
static s32
brcmf_cyw_cfg80211_get_tx_power(struct wiphy *wiphy, struct wireless_dev *wdev,
			    s32 *dbm)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_cyw_cfg80211_vif *vif = wdev_to_vif(wdev);
	struct brcmf_pub *drvr = cfg->pub;
	s32 qdbm = 0;
	s32 err;
...
static s32
brcmf_cyw_cfg80211_config_default_key(struct wiphy *wiphy, struct net_device *ndev,
...
static s32
brcmf_cyw_cfg80211_del_key(struct wiphy *wiphy, struct net_device *ndev,
...
static s32
brcmf_cyw_cfg80211_add_key(struct wiphy *wiphy, struct net_device *ndev,
		       u8 key_idx, bool pairwise, const u8 *mac_addr,
		       struct key_params *params)
{
	...
	if (params->key_len == 0)
		return brcmf_cyw_cfg80211_del_key(wiphy, ndev, key_idx, pairwise,
					      mac_addr);
...
static s32
brcmf_cyw_cfg80211_get_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_idx,
...
static s32
brcmf_cyw_cfg80211_config_default_mgmt_key(struct wiphy *wiphy,
...
static void
brcmf_cyw_cfg80211_reconfigure_wep(struct brcmf_if *ifp)
{
	struct brcmf_pub *drvr = ifp->drvr;
	s32 err;
	u8 key_idx;
...
static s32
brcmf_cyw_cfg80211_get_station_ibss(struct brcmf_if *ifp,
...
static s32
brcmf_cyw_cfg80211_get_station(struct wiphy *wiphy, struct net_device *ndev,
			   const u8 *mac, struct station_info *sinfo)
{
	...
	if (brcmf_is_ibssmode(ifp->vif))
		return brcmf_cyw_cfg80211_get_station_ibss(ifp, sinfo);
...
static int
brcmf_cyw_cfg80211_dump_station(struct wiphy *wiphy, struct net_device *ndev,
			    int idx, u8 *mac, struct station_info *sinfo)
{
	...
	if (idx < le32_to_cpu(cfg->assoclist.count)) {
		...
		return brcmf_cyw_cfg80211_get_station(wiphy, ndev, mac, sinfo);
	}
	return -ENOENT;
}

static s32
brcmf_cyw_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev,
			   bool enabled, s32 timeout)
{
	 ...
	 * cyw_cfg80211 even before the interface is up. In that
...
static s32 brcmf_inform_single_bss(struct brcmf_cyw_cfg80211_info *cfg,
				   struct brcmf_bss_info_le *bi)
{
	...
	u8 *notify_ie;
	size_t notify_ielen;
	struct cyw_cfg80211_inform_bss bss_data = {};
	...
	bss = cyw_cfg80211_inform_bss_data(wiphy, &bss_data,
	...
	cyw_cfg80211_put_bss(wiphy, bss);
...
static s32 brcmf_inform_bss(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pub *drvr = cfg->pub;
...
static s32 brcmf_inform_ibss(struct brcmf_cyw_cfg80211_info *cfg,
			     struct net_device *ndev, const u8 *bssid)
{
	...
	*(__le32 *)buf = cpu_to_le32(WL_BSS_INFO_MAX);
	...
	bss = cyw_cfg80211_inform_bss(wiphy, notify_channel,
...
static s32 brcmf_update_bss_info(struct brcmf_cyw_cfg80211_info *cfg,
				 struct brcmf_if *ifp)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_bss_info_le *bi;
...
void brcmf_abort_scanning(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct escan_info *escan = &cfg->escan_info;
...
static void brcmf_cyw_cfg80211_escan_timeout_worker(struct work_struct *work)
{
	struct brcmf_cyw_cfg80211_info *cfg =
			container_of(work, struct brcmf_cyw_cfg80211_info,
				     escan_timeout_work);
...
static s32
brcmf_compare_update_same_bss(struct brcmf_cyw_cfg80211_info *cfg,
...
static s32
brcmf_cyw_cfg80211_escan_handler(struct brcmf_if *ifp,
			     const struct brcmf_event_msg *e, void *data)
{
	struct brcmf_pub *drvr = ifp->drvr;
	struct brcmf_cyw_cfg80211_info *cfg = drvr->config;
	s32 status;
	...
	if (status == BRCMF_E_STATUS_PARTIAL) {
		...
		if (!cfg->int_escan_map && !cfg->scan_request) {
			brcmf_dbg(SCAN, "result without cyw_cfg80211 request\n");
			goto exit;
		}
...
static void brcmf_init_escan(struct brcmf_cyw_cfg80211_info *cfg)
{
	brcmf_fweh_register(cfg->pub, BRCMF_E_ESCAN_RESULT,
			    brcmf_cyw_cfg80211_escan_handler);
	cfg->escan_info.escan_state = WL_ESCAN_STATE_IDLE;
	/* Init scan_timeout timer */
	timer_setup(&cfg->escan_timeout, brcmf_escan_timeout, 0);
	INIT_WORK(&cfg->escan_timeout_work,
		  brcmf_cyw_cfg80211_escan_timeout_worker);
}

static struct cyw_cfg80211_scan_request *
brcmf_alloc_internal_escan_request(struct wiphy *wiphy, u32 n_netinfo) {
	struct cyw_cfg80211_scan_request *req;
	size_t req_size;
...
static int brcmf_internal_escan_add_info(struct cyw_cfg80211_scan_request *req,
 ...
 * scan request in the form of cyw_cfg80211_scan_request. For timebeing, create
 * cyw_cfg80211_scan_request one out of the received PNO event.
 */
static s32
				...
				const struct brcmf_event_msg *e, void *data)
{
	...
	struct cyw_cfg80211_scan_request *request = NULL;
	struct wiphy *wiphy = cfg_to_wiphy(cfg);
	int i, err = 0;
...
out_err:
	cyw_cfg80211_sched_scan_stopped(wiphy, 0);
free_req:
	kfree(request);
	return err;
}
...
static int
brcmf_cyw_cfg80211_sched_scan_start(struct wiphy *wiphy,
...
static int brcmf_cyw_cfg80211_sched_scan_stop(struct wiphy *wiphy,
...
static void brcmf_report_wowl_wakeind(struct wiphy *wiphy, struct brcmf_if *ifp)
{
	...
	struct cyw_cfg80211_wowlan_wakeup wakeup_data;
	...
	cyw_cfg80211_report_wowlan_wakeup(&ifp->vif->wdev, wakeup, GFP_KERNEL);
}
...
static s32 brcmf_cyw_cfg80211_resume(struct wiphy *wiphy)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct net_device *ndev = cfg_to_ndev(cfg);
	struct brcmf_if *ifp = netdev_priv(ndev);
	...
	struct brcmf_cyw_cfg80211_info *config = drvr->config;
	...
	if (cfg->wowl.active) {
		...
		if (cfg->wowl.nd_enabled) {
			brcmf_cyw_cfg80211_sched_scan_stop(cfg->wiphy, ifp->ndev, 0);
...
static void brcmf_configure_wowl(struct brcmf_cyw_cfg80211_info *cfg,
				 struct brcmf_if *ifp,
				 struct cyw_cfg80211_wowlan *wowl)
{
	u32 wowl_config;
	...
	if (wowl->nd_config) {
		brcmf_cyw_cfg80211_sched_scan_start(cfg->wiphy, ifp->ndev,
						wowl->nd_config);
...
static s32 brcmf_cyw_cfg80211_suspend(struct wiphy *wiphy,
				  struct cyw_cfg80211_wowlan *wowl)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct net_device *ndev = cfg_to_ndev(cfg);
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_vif *vif;
	struct brcmf_cyw_cfg80211_info *config = ifp->drvr->config;
	...
	if (brcmf_feat_is_enabled(ifp, BRCMF_FEAT_PNO))
		brcmf_cyw_cfg80211_sched_scan_stop(wiphy, ndev, 0);
...
exit:
	/* set cyw_cfg80211 pm state to cyw_cfg80211 suspended state */
...
static __used s32
brcmf_update_pmklist(struct brcmf_cyw_cfg80211_info *cfg, struct brcmf_if *ifp)
{
...
static s32
brcmf_cyw_cfg80211_set_pmksa(struct wiphy *wiphy, struct net_device *ndev,
...
static s32
brcmf_cyw_cfg80211_del_pmksa(struct wiphy *wiphy, struct net_device *ndev,
			 struct cyw_cfg80211_pmksa *pmksa)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
...
static s32
brcmf_cyw_cfg80211_flush_pmksa(struct wiphy *wiphy, struct net_device *ndev)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
	s32 err;
...
s32 brcmf_vif_set_mgmt_ie(struct brcmf_cyw_cfg80211_vif *vif, s32 pktflag,
...
s32 brcmf_vif_clear_mgmt_ies(struct brcmf_cyw_cfg80211_vif *vif)
{
	s32 pktflags[] = {
...
static s32
brcmf_config_ap_mgmt_ie(struct brcmf_cyw_cfg80211_vif *vif,
...
static s32
brcmf_cyw_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *ndev,
			struct cyw_cfg80211_ap_settings *settings)
{
	s32 ie_offset;
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	...
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_cyw_cfg80211_profile *profile = &ifp->vif->profile;
	struct cyw_cfg80211_crypto_settings *crypto = &settings->crypto;
	...
	if (dev_role == NL80211_IFTYPE_AP) {
		 ...
		 */
		brcmf_cyw_cfg80211_reconfigure_wep(ifp);
...
static int brcmf_cyw_cfg80211_stop_ap(struct wiphy *wiphy, struct net_device *ndev)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_pub *drvr = cfg->pub;
...
static s32
brcmf_cyw_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *ndev,
...
static int
brcmf_cyw_cfg80211_del_station(struct wiphy *wiphy, struct net_device *ndev,
...
static int
brcmf_cyw_cfg80211_change_station(struct wiphy *wiphy, struct net_device *ndev,
...
static void
brcmf_cyw_cfg80211_mgmt_frame_register(struct wiphy *wiphy,
				   struct wireless_dev *wdev,
...
static int
brcmf_cyw_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev,
		       struct cyw_cfg80211_mgmt_tx_params *params, u64 *cookie)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	...
	*cookie = 0;
	...
	if (ieee80211_is_probe_resp(mgmt->frame_control)) {
		...
		err = brcmf_vif_set_mgmt_ie(vif,
					    ...
					    &buf[ie_offset],
					    ie_len);
		cyw_cfg80211_mgmt_tx_status(wdev, *cookie, buf, len, true,
					GFP_KERNEL);
		...
		cyw_cfg80211_mgmt_tx_status(wdev, *cookie, buf, len, ack,
					GFP_KERNEL);
		kfree(af_params);
	} else {
...
static int
brcmf_cyw_cfg80211_cancel_remain_on_channel(struct wiphy *wiphy,
					struct wireless_dev *wdev,
					u64 cookie)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
...
static int brcmf_cyw_cfg80211_get_channel(struct wiphy *wiphy,
				      struct wireless_dev *wdev,
...
static int brcmf_cyw_cfg80211_crit_proto_start(struct wiphy *wiphy,
					   struct wireless_dev *wdev,
...
static void brcmf_cyw_cfg80211_crit_proto_stop(struct wiphy *wiphy,
					   struct wireless_dev *wdev)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_cyw_cfg80211_vif *vif;
...
static int brcmf_cyw_cfg80211_tdls_oper(struct wiphy *wiphy,
...
static int
brcmf_cyw_cfg80211_update_conn_params(struct wiphy *wiphy,
				  struct net_device *ndev,
				  struct cyw_cfg80211_connect_params *sme,
				  u32 changed)
{
...
#ifdef CONFIG_PM
static int
brcmf_cyw_cfg80211_set_rekey_data(struct wiphy *wiphy, struct net_device *ndev,
			      struct cyw_cfg80211_gtk_rekey_data *gtk)
{
	struct brcmf_cyw_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_if *ifp = netdev_priv(ndev);
...
static int brcmf_cyw_cfg80211_set_pmk(struct wiphy *wiphy, struct net_device *dev,
...
static int brcmf_cyw_cfg80211_del_pmk(struct wiphy *wiphy, struct net_device *dev,
				  const u8 *aa)
{
	struct brcmf_if *ifp;
...
static int
brcmf_cyw_cfg80211_change_bss(struct wiphy *wiphy, struct net_device *dev,
...
static struct cyw_cfg80211_ops brcmf_cyw_cfg80211_ops = {
	.add_virtual_intf = brcmf_cyw_cfg80211_add_iface,
	.del_virtual_intf = brcmf_cyw_cfg80211_del_iface,
	.change_virtual_intf = brcmf_cyw_cfg80211_change_iface,
	.scan = brcmf_cyw_cfg80211_scan,
	.set_wiphy_params = brcmf_cyw_cfg80211_set_wiphy_params,
	.join_ibss = brcmf_cyw_cfg80211_join_ibss,
	.leave_ibss = brcmf_cyw_cfg80211_leave_ibss,
	.get_station = brcmf_cyw_cfg80211_get_station,
	.dump_station = brcmf_cyw_cfg80211_dump_station,
	.set_tx_power = brcmf_cyw_cfg80211_set_tx_power,
	.get_tx_power = brcmf_cyw_cfg80211_get_tx_power,
	.add_key = brcmf_cyw_cfg80211_add_key,
	.del_key = brcmf_cyw_cfg80211_del_key,
	.get_key = brcmf_cyw_cfg80211_get_key,
	.set_default_key = brcmf_cyw_cfg80211_config_default_key,
	.set_default_mgmt_key = brcmf_cyw_cfg80211_config_default_mgmt_key,
	.set_power_mgmt = brcmf_cyw_cfg80211_set_power_mgmt,
	.connect = brcmf_cyw_cfg80211_connect,
	.disconnect = brcmf_cyw_cfg80211_disconnect,
	.suspend = brcmf_cyw_cfg80211_suspend,
	.resume = brcmf_cyw_cfg80211_resume,
	.set_pmksa = brcmf_cyw_cfg80211_set_pmksa,
	.del_pmksa = brcmf_cyw_cfg80211_del_pmksa,
	.flush_pmksa = brcmf_cyw_cfg80211_flush_pmksa,
	.start_ap = brcmf_cyw_cfg80211_start_ap,
	.stop_ap = brcmf_cyw_cfg80211_stop_ap,
	.change_beacon = brcmf_cyw_cfg80211_change_beacon,
	.del_station = brcmf_cyw_cfg80211_del_station,
	.change_station = brcmf_cyw_cfg80211_change_station,
	.sched_scan_start = brcmf_cyw_cfg80211_sched_scan_start,
	.sched_scan_stop = brcmf_cyw_cfg80211_sched_scan_stop,
	.mgmt_frame_register = brcmf_cyw_cfg80211_mgmt_frame_register,
	.mgmt_tx = brcmf_cyw_cfg80211_mgmt_tx,
	.remain_on_channel = brcmf_p2p_remain_on_channel,
	.cancel_remain_on_channel = brcmf_cyw_cfg80211_cancel_remain_on_channel,
	.get_channel = brcmf_cyw_cfg80211_get_channel,
	.start_p2p_device = brcmf_p2p_start_device,
	.stop_p2p_device = brcmf_p2p_stop_device,
	.crit_proto_start = brcmf_cyw_cfg80211_crit_proto_start,
	.crit_proto_stop = brcmf_cyw_cfg80211_crit_proto_stop,
	.tdls_oper = brcmf_cyw_cfg80211_tdls_oper,
	.update_connect_params = brcmf_cyw_cfg80211_update_conn_params,
	.set_pmk = brcmf_cyw_cfg80211_set_pmk,
	.del_pmk = brcmf_cyw_cfg80211_del_pmk,
	.change_bss = brcmf_cyw_cfg80211_change_bss,
};

struct cyw_cfg80211_ops *brcmf_cyw_cfg80211_get_ops(struct brcmf_mp_device *settings)
{
	struct cyw_cfg80211_ops *ops;

	ops = kmemdup(&brcmf_cyw_cfg80211_ops, sizeof(brcmf_cyw_cfg80211_ops),
		       GFP_KERNEL);
...
struct brcmf_cyw_cfg80211_vif *brcmf_alloc_vif(struct brcmf_cyw_cfg80211_info *cfg,
					   enum nl80211_iftype type)
{
	struct brcmf_cyw_cfg80211_vif *vif_walk;
	struct brcmf_cyw_cfg80211_vif *vif;
	bool mbss;
...
void brcmf_free_vif(struct brcmf_cyw_cfg80211_vif *vif)
{
	list_del(&vif->list);
	kfree(vif);
}

void brcmf_cyw_cfg80211_free_netdev(struct net_device *ndev)
{
	struct brcmf_cyw_cfg80211_vif *vif;
	struct brcmf_if *ifp;
...
static bool brcmf_is_linkup(struct brcmf_cyw_cfg80211_vif *vif,
...
static bool brcmf_is_nonetwork(struct brcmf_cyw_cfg80211_info *cfg,
			       const struct brcmf_event_msg *e)
{
	u32 event = e->event_code;
	u32 status = e->status;
...
static void brcmf_clear_assoc_ies(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_cyw_cfg80211_connect_info *conn_info = cfg_to_conn(cfg);
...
{
	struct brcmf_cyw_cfg80211_info *cfg = (struct brcmf_cyw_cfg80211_info *)config;
...
static void brcmf_wifi_prioritize_acparams(const
	struct brcmf_cyw_cfg80211_edcf_acparam *acp, u8 *priority)
{
	u8 aci;
	u8 aifsn;
	u8 ecwmin;
...
static s32 brcmf_get_assoc_ies(struct brcmf_cyw_cfg80211_info *cfg,
			       struct brcmf_if *ifp)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_cyw_cfg80211_assoc_ielen_le *assoc_info;
	struct brcmf_cyw_cfg80211_connect_info *conn_info = cfg_to_conn(cfg);
	struct brcmf_cyw_cfg80211_edcf_acparam edcf_acparam_info[EDCF_AC_COUNT];
	u32 req_len;
	u32 resp_len;
	s32 err = 0;
	...
	assoc_info =
		(struct brcmf_cyw_cfg80211_assoc_ielen_le *)cfg->extra_buf;
...
static s32
brcmf_bss_roaming_done(struct brcmf_cyw_cfg80211_info *cfg,
		       struct net_device *ndev,
		       const struct brcmf_event_msg *e)
{
	struct brcmf_if *ifp = netdev_priv(ndev);
	...
	struct brcmf_bss_info_le *bi;
	struct brcmu_chan ch;
	struct cyw_cfg80211_roam_info roam_info = {};
	u32 freq;
	s32 err = 0;
	u8 *buf;
	...
	*(__le32 *)buf = cpu_to_le32(WL_BSS_INFO_MAX);
	...
	cyw_cfg80211_roamed(ndev, &roam_info, GFP_KERNEL);
...
static s32
brcmf_bss_connect_done(struct brcmf_cyw_cfg80211_info *cfg,
		       ...
		       bool completed)
{
	...
	struct brcmf_cyw_cfg80211_profile *profile = &ifp->vif->profile;
	struct brcmf_cyw_cfg80211_connect_info *conn_info = cfg_to_conn(cfg);
	struct cyw_cfg80211_connect_resp_params conn_params;
	...
	if (test_and_clear_bit(BRCMF_VIF_STATUS_CONNECTING,
		...
		cyw_cfg80211_connect_done(ndev, &conn_params, GFP_KERNEL);
...
static s32
brcmf_notify_connect_status_ap(struct brcmf_cyw_cfg80211_info *cfg,
			       struct net_device *ndev,
			       const struct brcmf_event_msg *e, void *data)
{
	struct brcmf_pub *drvr = cfg->pub;
	...
	if (((event == BRCMF_E_ASSOC_IND) || (event == BRCMF_E_REASSOC_IND)) &&
		...
		cyw_cfg80211_new_sta(ndev, e->addr, sinfo, GFP_KERNEL);

		kfree(sinfo);
	} else if ((event == BRCMF_E_DISASSOC_IND) ||
		   (event == BRCMF_E_DEAUTH_IND) ||
		   (event == BRCMF_E_DEAUTH)) {
		cyw_cfg80211_del_sta(ndev, e->addr, GFP_KERNEL);
	}
	return 0;
}
			    ...
			    const struct brcmf_event_msg *e, void *data)
{
	...
	if (brcmf_is_apmode(ifp->vif)) {
		...
		if (brcmf_is_ibssmode(ifp->vif)) {
			...
			memcpy(profile->bssid, e->addr, ETH_ALEN);
			cyw_cfg80211_ibss_joined(ndev, e->addr, chan, GFP_KERNEL);
			...
			const struct brcmf_event_msg *e, void *data)
{
	...
	cyw_cfg80211_michael_mic_failure(ifp->ndev, (u8 *)&e->addr, key_type, -1,
				     NULL, GFP_KERNEL);
				  ...
				  const struct brcmf_event_msg *e, void *data)
{
	...
	struct brcmf_cyw_cfg80211_vif_event *event = &cfg->vif_event;
	struct brcmf_cyw_cfg80211_vif *vif;
...
static void brcmf_init_conf(struct brcmf_cyw_cfg80211_conf *conf)
{
...
static void brcmf_register_event_handlers(struct brcmf_cyw_cfg80211_info *cfg)
{
...
static void brcmf_deinit_priv_mem(struct brcmf_cyw_cfg80211_info *cfg)
{
	kfree(cfg->conf);
	cfg->conf = NULL;
	kfree(cfg->extra_buf);
...
static s32 brcmf_init_priv_mem(struct brcmf_cyw_cfg80211_info *cfg)
{
	...
	cfg->wowl.nd_info = kzalloc(sizeof(*cfg->wowl.nd_info) +
				    sizeof(struct cyw_cfg80211_wowlan_nd_match *),
				    GFP_KERNEL);
	if (!cfg->wowl.nd_info)
		goto init_priv_mem_out;
...
static s32 wl_init_priv(struct brcmf_cyw_cfg80211_info *cfg)
{
	s32 err = 0;
...
static void wl_deinit_priv(struct brcmf_cyw_cfg80211_info *cfg)
{
...
static void init_vif_event(struct brcmf_cyw_cfg80211_vif_event *event)
{
...
static int brcmf_construct_chaninfo(struct brcmf_cyw_cfg80211_info *cfg,
				    u32 bw_cap[])
{
	struct wiphy *wiphy = cfg_to_wiphy(cfg);
	struct brcmf_pub *drvr = cfg->pub;
...
static int brcmf_enable_bw40_2g(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_if *ifp = brcmf_get_ifp(drvr, 0);
	struct ieee80211_supported_band *band;
...
static int brcmf_setup_wiphybands(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_if *ifp = brcmf_get_ifp(drvr, 0);
	struct wiphy *wiphy = cfg_to_wiphy(cfg);
...
static void brcmf_wiphy_wowl_params(struct wiphy *wiphy, struct brcmf_if *ifp)
{
	...
	struct cyw_cfg80211_wowlan *brcmf_wowlan_config = NULL;
...
static s32 brcmf_config_dongle(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct net_device *ndev;
	struct wireless_dev *wdev;
	...
	err = brcmf_cyw_cfg80211_change_iface(wdev->wiphy, ndev, wdev->iftype,
					  NULL);
	if (err)
		goto default_conf_out;
...
static s32 __brcmf_cyw_cfg80211_up(struct brcmf_if *ifp)
{
...
static s32 __brcmf_cyw_cfg80211_down(struct brcmf_if *ifp)
{
	struct brcmf_cyw_cfg80211_info *cfg = ifp->drvr->config;
...
s32 brcmf_cyw_cfg80211_up(struct net_device *ndev)
{
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_info *cfg = ifp->drvr->config;
	s32 err = 0;
	...
	err = __brcmf_cyw_cfg80211_up(ifp);
	mutex_unlock(&cfg->usr_sync);

	return err;
}

s32 brcmf_cyw_cfg80211_down(struct net_device *ndev)
{
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_cyw_cfg80211_info *cfg = ifp->drvr->config;
	s32 err = 0;
	...
	mutex_lock(&cfg->usr_sync);
	err = __brcmf_cyw_cfg80211_down(ifp);
	mutex_unlock(&cfg->usr_sync);

	return err;
}

enum nl80211_iftype brcmf_cyw_cfg80211_get_iftype(struct brcmf_if *ifp)
{
...
bool brcmf_get_vif_state_any(struct brcmf_cyw_cfg80211_info *cfg,
			     unsigned long state)
{
	struct brcmf_cyw_cfg80211_vif *vif;
...
static inline bool vif_event_equals(struct brcmf_cyw_cfg80211_vif_event *event,
				    u8 action)
{
	u8 evt_action;
...
void brcmf_cyw_cfg80211_arm_vif_event(struct brcmf_cyw_cfg80211_info *cfg,
				  struct brcmf_cyw_cfg80211_vif *vif)
{
	struct brcmf_cyw_cfg80211_vif_event *event = &cfg->vif_event;
...
bool brcmf_cyw_cfg80211_vif_event_armed(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_cyw_cfg80211_vif_event *event = &cfg->vif_event;
	bool armed;
...
int brcmf_cyw_cfg80211_wait_vif_event(struct brcmf_cyw_cfg80211_info *cfg,
...
static void brcmf_cyw_cfg80211_reg_notifier(struct wiphy *wiphy,
...
struct brcmf_cyw_cfg80211_info *brcmf_cyw_cfg80211_attach(struct brcmf_pub *drvr,
						  struct cyw_cfg80211_ops *ops,
						  bool p2pdev_forced)
{
	...
	wiphy->reg_notifier = brcmf_cyw_cfg80211_reg_notifier;
	wiphy->regulatory_flags |= REGULATORY_CUSTOM_REG;
	wiphy_apply_custom_regulatory(wiphy, &brcmf_regdom);

	/* firmware defaults to 40MHz disabled in 2G band. We signal
	 * cyw_cfg80211 here that we do and have it decide we can enable
...
#ifdef CONFIG_PM
	if (brcmf_feat_is_enabled(ifp, BRCMF_FEAT_WOWL_GTK))
		ops->set_rekey_data = brcmf_cyw_cfg80211_set_rekey_data;
#endif
	...
	/* If cyw_cfg80211 didn't disable 40MHz HT CAP in wiphy_register(),
...
void brcmf_cyw_cfg80211_detach(struct brcmf_cyw_cfg80211_info *cfg)
{
	if (!cfg)
		return;
