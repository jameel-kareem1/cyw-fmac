...
/**
 * brcmf_p2p_scan_is_p2p_request() - is cyw_cfg80211 scan request a P2P scan.
 *
 * @request: the scan request as received from cyw_cfg80211.
 *
 * returns true if one of the ssids in the request matches the
 * P2P wildcard ssid; otherwise returns false.
 */
static bool brcmf_p2p_scan_is_p2p_request(struct cyw_cfg80211_scan_request *request)
{
	struct cyw_cfg80211_ssid *ssids = request->ssids;
	int i;
 ...
 * @request: scan request from cyw_cfg80211.
 * @action: scan action.
 *
 * Determines the P2P discovery state based to scan request parameters and
 * validates the channels in the request.
 */
static s32 brcmf_p2p_run_escan(struct brcmf_cyw_cfg80211_info *cfg,
			       struct brcmf_if *ifp,
...
static s32 brcmf_p2p_find_listen_channel(const u8 *ie, u32 ie_len)
{
	...
	err = cyw_cfg80211_get_p2p_attr(ie, ie_len,
				...
				unsigned int duration, u64 *cookie)
{
	...
	*cookie = p2p->remain_on_channel_cookie;
	cyw_cfg80211_ready_on_channel(wdev, *cookie, channel, duration, GFP_KERNEL);
				     ...
				     void *data)
{
	...
	if (test_and_clear_bit(BRCMF_P2P_STATUS_DISCOVER_LISTEN,
		...
		cyw_cfg80211_remain_on_channel_expired(wdev,
						   p2p->remain_on_channel_cookie,
...
static s32 brcmf_p2p_af_searching_channel(struct brcmf_p2p_info *p2p)
{
	...
	struct brcmf_cyw_cfg80211_vif *pri_vif;
	s32 retry;
...

{
	...
	err = cyw_cfg80211_get_p2p_attr(ie, le32_to_cpu(bi->ie_length),
 ...
 *
 */
static s32 brcmf_p2p_abort_action_frame(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_p2p_info *p2p = &cfg->p2p;
	struct brcmf_cyw_cfg80211_vif *vif;
	s32 err;
 ...
 */
static void
brcmf_p2p_stop_wait_next_action_frame(struct brcmf_cyw_cfg80211_info *cfg)
{
	struct brcmf_p2p_info *p2p = &cfg->p2p;
...
{
	struct brcmf_cyw_cfg80211_info *cfg = p2p->cfg;
	struct brcmf_if *ifp;
				     ...
				     void *data)
{
	...
	wdev = &ifp->vif->wdev;
	cyw_cfg80211_rx_mgmt(wdev, freq, 0, (u8 *)mgmt_frame, mgmt_frame_len, 0);
 ...
 */
static s32 brcmf_p2p_pub_af_tx(struct brcmf_cyw_cfg80211_info *cfg,
			       struct brcmf_fil_af_params_le *af_params,
			       struct brcmf_config_af_params *config_af_params)
{
	...
	case P2P_PAF_PROVDIS_REQ:
		...
		if (cyw_cfg80211_get_p2p_attr(&act_frm->elts[0], ie_len,
					  ...
					  void *data)
{
	...
	cyw_cfg80211_rx_mgmt(&vif->wdev, freq, 0, mgmt_frame, mgmt_frame_len, 0);
		       ...
		       enum brcmf_fil_p2p_if_types if_type)
{
	...
	err = brcmf_cyw_cfg80211_wait_vif_event(cfg, BRCMF_E_IF_CHANGE,
...
static int brcmf_p2p_disable_p2p_if(struct brcmf_cyw_cfg80211_vif *vif)
{
	struct brcmf_cyw_cfg80211_info *cfg = wdev_to_cfg(&vif->wdev);
...
static int brcmf_p2p_release_p2p_if(struct brcmf_cyw_cfg80211_vif *vif)
{
	struct brcmf_cyw_cfg80211_info *cfg = wdev_to_cfg(&vif->wdev);
	struct net_device *pri_ndev = cfg_to_ndev(cfg);
	struct brcmf_if *ifp = netdev_priv(pri_ndev);
...
{
	struct brcmf_pub *drvr = p2p->cfg->pub;
	struct brcmf_cyw_cfg80211_vif *p2p_vif;
	...
	brcmf_cyw_cfg80211_arm_vif_event(p2p->cfg, p2p_vif);
	...
	if (err < 0) {
		...
		brcmf_cyw_cfg80211_arm_vif_event(p2p->cfg, NULL);
		goto fail;
	}

	/* wait for firmware event */
	err = brcmf_cyw_cfg80211_wait_vif_event(p2p->cfg, BRCMF_E_IF_ADD,
					    BRCMF_VIF_EVENT_TIMEOUT);
	brcmf_cyw_cfg80211_arm_vif_event(p2p->cfg, NULL);
	brcmf_fweh_p2pdev_setup(pri_ifp, false);
	if (!err) {
...
int brcmf_p2p_get_conn_idx(struct brcmf_cyw_cfg80211_info *cfg)
{
	int i;
...
int brcmf_p2p_del_vif(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (!err) {
		/* wait for firmware event */
		err = brcmf_cyw_cfg80211_wait_vif_event(cfg, BRCMF_E_IF_DEL,
						    BRCMF_VIF_EVENT_TIMEOUT);
		if (!err)
			err = -EIO;
		else
...
void brcmf_p2p_ifp_removed(struct brcmf_if *ifp, bool rtnl_locked)
{
	...
	if (!rtnl_locked)
		rtnl_lock();
	cyw_cfg80211_unregister_wdev(&vif->wdev);
	if (!rtnl_locked)
		rtnl_unlock();
	brcmf_free_vif(vif);
}
